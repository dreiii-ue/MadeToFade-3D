...
foreach ($default_payment_methods as $method) {
    ...
    $instructions = "Please follow the payment instructions before submitting your order.";

    if ($method == "Cash on Delivery") {
        $instructions = "Pay the courier once your order is out for delivery.";
    }

    mysqli_query(
        $conn,
        "INSERT IGNORE INTO payment_settings(payment_method, account_name, instructions, is_active)
         VALUES('$safe_method', 'Made To Fade', '$instructions', 'Yes')"
    );
}

$active_payment_methods = mysqli_query(
    $conn,
    "SELECT payment_method
     FROM payment_settings
     WHERE is_active='Yes'
     ORDER BY FIELD(payment_method, 'Cash on Delivery', 'GCash', 'Maya', 'QRPH', 'Bank Transfer'), payment_method ASC"
);
...
if (isset($_POST['add_cart'])) {
    ...
    if (mysqli_num_rows($check) > 0) {
        ...
        $new_qty = (int)$cart_row['quantity'] + $quantity;
...
$total = 0;
$has_payment_methods = mysqli_num_rows($active_payment_methods) > 0;
?>
<!DOCTYPE html>
<html>
<head>
                        ...
                        <td>
                            <form method="POST" class="qty-control">
                                ...
                                <button
                                    ...
                                    <?php if ((int)$row['quantity'] >= (int)$row['stock']) echo "disabled"; ?>
                                >
                                    +
                                </button>
        ...
        <div class="panel">
            ...
            <form method="POST" action="checkout.php" class="checkout-form checkout-details-form">
                ...
                <div>
                    ...
                    <select name="payment_method" required <?php if (!$has_payment_methods) echo "disabled"; ?>>
                        <option value="">Select payment method</option>

                        <?php while ($method = mysqli_fetch_assoc($active_payment_methods)) { ?>
                ...
                <div class="checkout-button-box">
                    <button
                        ...
                        name="checkout"
                        class="btn"
                        <?php if (!$has_payment_methods) echo "disabled"; ?>
                    >
                        Checkout
                    </button>
                </div>
